...
Fix $k\ge2$ and let $P_0$ be such that every prime $\ell>P_0$ satisfies $\ell>2k$
(equivalently $P_0\ge 2k-1$). Let $n$ satisfy
\begin{enumerate}
...
\begin{remark}
The hypothesis on $P_0$ is load-bearing, not cosmetic: if a prime $\ell\le 2k$ is allowed to
exceed $P_0$ it may divide two window elements and the demand at $\ell$ exceeds $2$.
Condition (ii) is sufficient but not necessary (carries at higher digit positions also count).
Numerically, with $P_0=2k$: over all even $n\le6\cdot10^4$ the predicate has \emph{zero} false
positives and certifies $45$ of the $263$ even members of $S_2$; for $k=3$ it certifies $10$ of
the $41$ members of $S_3\cap[1,6\cdot10^4]$, including the least member $3475$. An independent
adversarial audit reproduced zero false positives for Lemma R$'''$ up to $n=10^6$ and $k=2,\dots,6$.
A prime-parametrized specialization, in which each window element is a balanced semiprime in an
explicit ratio box, was verified on $123$ solutions and independently regenerated.
\end{remark}
...
There is an absolute constant $c>0$ such that $F(u)\ \ge\ c\,2^{-u}/u$ for every $u\ge2$,
while $S(u)\le\rho(u)=u^{-u(1+o(1))}$. Consequently $S(u)/F(u)\to0$ as $u\to\infty$: for every
sufficiently large $u$, strictly more $n\le X$ fail some carry condition at a prime
$\le n^{1/u}$ than have an $n^{1/u}$-smooth window. Hence no unconditional first-moment or
union-bound argument over all $n$ can produce a member of $S_2$ at any large threshold $u$.
\end{proposition}

\begin{proof}
\emph{Upper bound for $S(u)$.} $S(u)$ is at most the density of $n$ with $n+1$ being
$n^{1/u}$-smooth, which is $\rho(u)+o(1)$ by the Dickman--de Bruijn theorem, and
$\rho(u)=u^{-u(1+o(1))}$.

\emph{Lower bound for $F(u)$.} Fix $j=1$. If $p\,\|\,n+1$ with $4<p$ and $W=(n+1)/p$, then
$n=p(W-1)+(p-1)$, so the base-$p$ digit of $n$ at position $0$ is $p-1$, which carries and
passes carry $1$ to position~$1$; the digits of $n$ above position~$0$ are those of $W-1$.
Hence $\kappa_p(n)=1+\gamma$, where $\gamma$ is the number of carries when doubling $W-1$ in
base $p$ with carry-in $1$, while the demand is $2$. So $n$ fails at $p$ exactly when
$\gamma=0$, i.e.\ when the base-$p$ digits $d_0,d_1,\dots$ of $W-1$ satisfy
$2d_0+1<p$ and $2d_i<p$ for $i\ge1$; call such $W-1$ \emph{digit-poor}. Among the residues
modulo $p^{D}$ the digit-poor ones number at least $\lceil (p-1)/2\rceil^{D}\ge (p/2-1)^D$, a
proportion $\ge 2^{-D}(1-2/p)^D$.

Now let $p$ range over primes with $p=X^{\beta}$, $\beta\le 1/u$, and let $n\le X$. For fixed
$p$, the pairs $(p,W)$ with $n+1=pW\le X$, $p\nmid W$ and $W-1$ digit-poor number
$\gg (X/p)\,2^{-D}$ with $D=\lfloor\log(X/p)/\log p\rfloor$, i.e.\ $D\ge 1/\beta-2$. Summing
$1/p$ over the primes in a range $\beta\in[\beta_1,\beta_2]$ contributes $\log(\beta_2/\beta_1)$
by Mertens, so
\[
 \sum_{p\le X^{1/u}} \frac{1}{p}\,2^{-D(p)} \;\gg\; \int_0^{1/u} 2^{-(1/\beta-2)}\frac{d\beta}{\beta}
 \;=\;\int_u^{\infty} 2^{-(t-2)}\frac{dt}{t}\;\gg\;\frac{2^{-u}}{u}.
\]
These events for distinct $p$ overlap only in the second-order terms: by Bonferroni the density
of their union is at least $\Sigma-\Sigma^2/2$ where $\Sigma$ is the above sum, and $\Sigma<1$
for $u\ge2$, so the union has density $\gg 2^{-u}/u$. (The condition $p\,\|\,n+1$ rather than
$p\mid n+1$ costs only a factor $1-1/p$.) This gives $F(u)\ge c\,2^{-u}/u$.

Since $u^{-u(1+o(1))}\big/\big(c\,2^{-u}/u\big)=\big((2/u)^{u}\big)^{1+o(1)}\to0$, the ratio
$S(u)/F(u)$ tends to $0$.
\end{proof}

\noindent The asymptotic statement is what Proposition~\ref{prop:fm} proves; the small-$u$
regime, where it is not yet decisive, was measured exhaustively for $n\le2\cdot10^5$ using the
\emph{exact} failure condition $\kappa_p(n)<2\nu_p(n+j)$
(\texttt{experiments/first\_moment\_obstruction.py}):
\[
\begin{array}{c|ccccc}
u & 2.0 & 2.5 & 3.0 & 4.0 & 5.0\\\hline
F(u) & 0.4642 & 0.3242 & 0.2377 & 0.1410 & 0.0778\\
S(u) & 0.0592 & 0.0091 & 0.0009 & 0.00002 & \approx0\\
\rho(u) & 0.3069 & 0.1303 & 0.0486 & 0.0049 & 0.0003\\
S/F & 0.128 & 0.028 & 0.004 & 0.0001 & \approx0
\end{array}
\]
Note that already at $u=2$ one has $F(u)>\rho(u)\ge S(u)$, so the obstruction is decisive at
every threshold in the measured range, not merely asymptotically. At $u=2$, $98.96\%$ of all
$n\le3\cdot10^5$ fail some condition, against a smooth-window density of
$9.37\%\approx\rho(2)^2$. Moreover the conditions are positively correlated in failure: the
observed conditional membership rate is $0.1105$ against $0.3297$ predicted by an independence
model, so even an independence heuristic overstates the target threefold.
...
\bibitem{Bal29} Balakran (1929), cited as [Ba29] on erdosproblems.com/727 for the theorem that
  $S_1$ is infinite. \emph{Exact bibliographic details not independently verified in this run;
  only the statement is used, and it is re-derived constructively in
  \texttt{attempts/route-R3/FAMILY.md}.}
