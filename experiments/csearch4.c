...
static void build_universe(void) {
    ...
    if (f) { for (int n=2;n<=N;n++) allowed[n]=0; int x; while (fscanf(f,"%d",&x)==1) if (x>=2&&x<=N) allowed[x]=1; fclose(f); return; }
...
static void dfs(int pos, u128 R, int runlen) {
    ...
    if (pos > N) return;
    if (nch + navail[pos] < MINSZ) return;
