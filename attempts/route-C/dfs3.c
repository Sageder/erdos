...
static int N, budget, prefer_skip=0, minelts=0;
static long long tnum=1, tden=1;
static int navail[4098];
static u128 bot[4098];
...
static void dfs(int pos, u128 R, int runlen, int iso) {
    ...
    if (R % Q[pos]) return;
    if (nch + navail[pos] < minelts) return;
    { int need = minelts - nch; if (need > 0) { if (need > navail[2]) return; if (R < bot[need]) return; } }
    int extra = (runlen == 1) ? 1 : 0;
    if (prefer_skip) {
...
int main(void) {
    int k;
    if (scanf("%d %d %lld %d %d %lld %lld", &N, &budget, &maxsol, &prefer_skip, &minelts, &tnum, &tden) != 7) return 1;
    ...
    { long long d = tden;
      for (int pi = 0; pi < np; pi++) { int p = primes[pi];
          int need = 0; long long t = d; while (t % p == 0) { t /= p; need++; }
          int have = 0; { u128 x = L; while (x % (u128)p == 0) { x /= (u128)p; have++; } }
          for (int i = have; i < need; i++) L *= (u128)p; }
      long long t2 = d; for (int pi=0; pi<np; pi++){int p=primes[pi]; while(t2%p==0) t2/=p;}
      if (t2 != 1) { fprintf(stderr,"tden has a prime factor > N\n"); return 3; } }
    ...
    navail[N + 1] = 0;
    for (int n = N; n >= 2; n--) navail[n] = navail[n + 1] + (allowed[n] ? 1 : 0);
    { int j = 0; bot[0] = 0;
      for (int n = N; n >= 2; n--) if (allowed[n]) { j++; bot[j] = bot[j-1] + w[n]; } }
    dfs(2, (L / (u128)tden) * (u128)tnum, 0, 0);
