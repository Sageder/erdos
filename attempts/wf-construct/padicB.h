...
static int PA_ovr_n = 0; static int PA_ovr_p[4096]; static long long PA_ovr_t[4096];
static long long pa_override(int p, int *found)
{ for (int t = 0; t < PA_ovr_n; t++) if (PA_ovr_p[t] == p) { *found = 1; return PA_ovr_t[t]; }
  *found = 0; return 0; }
static int padic_init(int cnt, const int *el, int Nhi, long long u, long long v, int gadget)
{
    ...
    for (int p = 2; p <= Nhi; p++) {
        ...
        if (gadget) PA_tg[j] = 0;
            ...
            int fnd = 0; long long t0 = pa_override(p, &fnd);
            if (!fnd) { fprintf(stderr, "missing p-adic override for p=%d\n", p); exit(1); }
            PA_tg[j] = t0 % M;
        }
        else {
