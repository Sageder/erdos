/* pairsearch.c -- find legal U whose maximal runs ALL have length exactly 2, i.e.
...
static void dfs(int a, u128 R, int j) {
    ...
    if (j == 0) {
        if (R == 0) {
            ...
            printf("\n"); fflush(stdout);
            if (nsol >= maxsol) { printf("(cap)\n"); fflush(stdout); }
        }
        return;
    }
    if (a > N) return;
    if (nsol >= maxsol) return;
    if (MX(a, j) < R) return;            /* cannot reach R any more */
    if (MN(a, j) > R) return;            /* would overshoot */
    /* p-adic check for primes whose multiple a-1 was just passed */
    for (int i = 0; i < ndp[a - 1]; i++) if (!checkp(dprimes[a - 1][i], a)) return;

    if (isPairStart[a] && wp[a] <= R) {
        int sv[16], nsv = 0;
        for (int d = 0; d < 2; d++) {
            int n = a + d;
        ...
        chosen[nch++] = a;
        /* positions a+1 (in the pair) and a+2 (must stay empty) are skipped;
           the p-adic bookkeeping for a+2 needs no update since it is not chosen */
        dfs(a + 3, R - wp[a], j - 1);
        nch--;
...
int main(int argc, char **argv) {
    ...
    for (int k = kmin; k <= kmax && k <= MAXJ; k++) {
        ...
        nodes = 0; nsol = 0; nch = 0; targetK = k;
        dfs(2, L, k);
        printf("# k=%d pairs: solutions>=%lld nodes=%lld %s\n", k, nsol, nodes,
               nsol == 0 ? "NONE-in-pairs-class" : "");
        fflush(stdout);
    }
    return 0;
}
